...
class MnistEnv(gym.Env):
    ...
    def __init__(self, start=None, goal=None, objects=None, viz=False, discrete_progress=False):
        self.viz = viz
        self.robot_size = 1.
        self.goal_size  = 1.
        ...
        self.state = start
        self.reward_fn = rw.make_default_reward(goal)
        ## self.rewards   = None
        self.trees = None
        self.discrete_progress=discrete_progress

        if viz: _, self.ax = plt.subplots()
    ...
    def observation_space(self):
        return spaces.Box( 0., 28., (2,), dtype=np.float32)
